...
Go version is pinned to **1.26.7** (see `go.mod`; CI reads it via `go-version-file` in the `.github/actions/setup-go` composite action). `.goreleaser.yml` builds release artifacts (CGO disabled, multi-arch).
