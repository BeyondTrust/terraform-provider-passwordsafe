...
func TestDeleteManagedSystemByAsset(t *testing.T) {
	// mocking Password Safe API
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case constants.APIPath + "/Assets/5/ManagedSystems":
			if r.Method == http.MethodPost {
				_, err := w.Write([]byte(`{"ManagedSystemID": 13, "EntityTypeID": 1, "AssetID": 1, "SystemName": "test-system"}`))
				if err != nil {
					t.Error(err.Error())
				}
			}
	...
	configManagedSystem := entities.PasswordSafeTestConfig{
		...
		URL:                          server.URL,
		Resource: `
		resource "passwordsafe_managed_system_by_asset" "test_managed_system" {
			asset_id                               = "5"
			platform_id                            = 2
			contact_email                          = "admin@example.com"
			description                            = "Test managed system for deletion"
			port                                   = 5432
			timeout                                = 30
			ssh_key_enforcement_mode               = 1
			password_rule_id                       = 0
		...
		Steps: []resource.TestStep{
			{
				...
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(
						"passwordsafe_managed_system_by_asset.test_managed_system",
						tfjsonpath.New("managed_system_id"),
						knownvalue.Int32Exact(13),
					),
				},
