...
func TestManageAccountFlowFailedRequests(t *testing.T) {

	testConfig := ManagedAccountCreateRequestConfig{
		name: "TestManageAccountFlowFailedRequests",
		server: httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			// Mocking Response accorging to the endpoint path
			switch r.URL.Path {
		...
		response: `got a non 200 status code: 400 - "Missing request body"`,
	}
...
func TestManageAccountFlowFailedCredentials(t *testing.T) {

	testConfig := ManagedAccountCreateRequestConfig{
		name: "TestManageAccountFlowFailedCredentials",
		server: httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			// Mocking Response accorging to the endpoint path
			switch r.URL.Path {
			...
			case "/Credentials/124":
				w.WriteHeader(http.StatusNotFound)
				w.Write([]byte(`"Request has been released or is expired"`))
		...
		response: `got a non 200 status code: 404 - "Request has been released or is expired"`,
	}
